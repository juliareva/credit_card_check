...
function validateCred (arr) {
  ...
  tempArr = tempArr.map(x => x > 9 ? x % 10 + 1 : x)   

  for (let j = arr.length - 1; j >= 0; j -=2) {
    tempArr2.push(arr[j])
  }

  let sum = tempArr.reduce((acc, current) => acc + current, 0) 
  + tempArr2.reduce((acc, current) => acc + current, 0) ;
...
// Find invalid cards' numbers of the batch
...
// Find invalid cards' companies and remove duplicates
